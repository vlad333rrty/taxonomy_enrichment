...
from datasets.FasttextVectorizer import FasttextVectorizer
from datasets.TaxoFormatter import TaxoFormatter, TaxoInferFormatter
from datasets.wordnet_parser import WordnetParser
...
def prepare_wordnet_for_training(wn_path: str, train_ratio: float):
    start = time.time()
    delta, wn_redaer = common.performance.measure(lambda: WordNetCorpusReader(wn_path, None))
    print('Wordnet reader created in', delta, 'seconds')

    wordnet_parser = WordnetParser(wn_redaer)
    delta, terms_and_relations = common.performance.measure(lambda: wordnet_parser.traverse_nouns())
    ...
    delta, model = common.performance.measure(
    ...
    delta, term_and_embed = common.performance.measure(lambda: vectorizer.vectorize_terms(terms))
...
# prepare_terms_for_inference('data/datasets/diachronic-wordnets/en/no_labels_nouns_en.2.0-3.0.tsv')
