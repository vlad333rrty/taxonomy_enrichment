...
def __enrich_submitted(submitted):
...
def main():
    ...
    reference = read_dataset(args.reference, lambda x: x.split(','))
    submitted = read_dataset(args.predicted, lambda x: x.split(','), ' ')
    reference = {k : v for k,v in filter(lambda x: x[0] in submitted,reference.items())}
    submitted = __enrich_submitted(submitted)
    ...
    map_list, mrr_list = get_score_list(reference, submitted, args.predicted, k=20)
